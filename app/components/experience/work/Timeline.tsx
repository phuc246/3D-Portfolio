...
const TimelinePoint = ({ point, diff }: { point: WorkTimelinePoint, diff: number }) => {
  ...
  return (
    <group position={point.point} scale={isMobile ? 0.35 : 0.6}>
      ...
      <group>
        <group position={getPoint} scale={[0.7 + 0.3 * (1 - diff), 0.7 + 0.3 * (1 - diff), 1]}>
          <Text {...textProps} fontSize={0.3} position={[-diff / 2, 0, 0]} fillOpacity={1 - diff}>
            {point.year}
          </Text>
          <group position={[0, -0.7, 0]}>
            <Text {...titleProps} fontSize={0.5} maxWidth={3} position={[0, 0, 0]} fillOpacity={1 - diff}>
              {point.title}
            </Text>
            <Text {...textProps} fontSize={0.18} position={[0, -0.6, 0]} fillOpacity={1 - diff}>
              {point.subtitle}
            </Text>
          </group>
        </group>
